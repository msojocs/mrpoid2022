...
int32 mr_socket(int32 type, int32 protocol) {
    ...
    sock_fd = socket(AF_INET, newType, 0);
    if (sock_fd >= 0) {
        socStat[index].socketId = sock_fd;
        ...
        //设置异步模式
        if ((ret = fcntl(sock_fd, F_GETFL, 0)) < 0) {
            LOGE("  socket unblock err1.");
        }
        if (fcntl(sock_fd, F_SETFL, ret | O_NONBLOCK) < 0) {
            LOGE("  socket unblock err2.");
        }

        socket_set_keepAlive(sock_fd);
...
int32 mr_recvfrom(int32 s, char *buf, int len, int32 *ip, uint16 *port) {
    ...
    if (socStat[s].readStat == DSM_SOC_READABLE) {
        if (ip != NULL && port != NULL) {
            ...
            read = recvfrom(socStat[s].socketId, buf, len, 0, (struct sockaddr *) &fromaddr,
                            &addr_len);
...
int32 mr_sendto(int32 s, const char *buf, int len, int32 ip, uint16 port) {
    ...
    if (socStat[s].writeStat == DSM_SOC_WRITEABLE) {
        ...
        write = sendto(socStat[s].socketId, buf, len, 0, (struct sockaddr *) &toaddr,
